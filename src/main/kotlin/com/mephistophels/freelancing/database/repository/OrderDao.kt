...
interface OrderDao : AppRepository<Order>, PagingAndSortingRepository<Order, Long>, CrudRepository<Order, Long> {
    ...
    @Transactional
    fun updateStatusById(status: OrderStatus,
                         id: Long,
                         )
    ...
    @Transactional
    fun updateExecutorIdById(executorId:Long,
                             id:Long,
                             )

    @Modifying
    @Query(value = "insert into ExecutorRequestToOrderTable (orderId, executorId) values (:orderId, :executorId)", nativeQuery = true)
    @Transactional
    fun addExecutorRequest(@Param("orderId") orderId: Long,
